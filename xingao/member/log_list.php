...
$headtitle=cadd($_GET['key'])." 会员登录日志";
...
//echo $search;exit();
$query="select * from member_log where {$where} {$order}";
...
<!-- BEGIN PAGE TITLE & BREADCRUMB-->
      ...
      <button type="button" class="btn btn-danger" onClick="javascript:if(confirm('确认要删除吗?此操作不可恢复!'))location.href='log_save.php?lx=del&date=12';" ><i class="icon-remove"></i> <?=$LG['del']?>所有会员12个月前日志 </button>
      <button type="button" class="btn btn-danger" onClick="javascript:if(confirm('确认要删除吗?此操作不可恢复!'))location.href='log_save.php?lx=del&date=6';"><i class="icon-remove"></i> <?=$LG['del']?>所有会员6个月前日志 </button>
      <button type="button" class="btn btn-danger" onClick="javascript:if(confirm('确认要删除吗?此操作不可恢复!'))location.href='log_save.php?lx=del&date=3';"><i class="icon-remove"></i> <?=$LG['del']?>所有会员3个月前日志 </button>
      </ul>
        ...
        <div class="collapse navbar-collapse navbar-ex1-collapse">
          ...
          <input name="my" type="hidden" value="<?=$my?>">
            <div class="form-group">
              <input type="text" name="key" class="form-control input-msmall popovers" data-trigger="hover" data-placement="right"  data-content="会员ID/会员名/登录IP (可留空)" value="<?=$key?>">
            </div>
            <div class="form-group">
                <div class="col-md-0">
          ...
          <thead>
            <tr>
              <th align="center"><a href="?<?=$search?>&orderby=logintime&orderlx=" class="<?=orac('logintime')?>">登录时间</a></th>
                <th align="center"><a href="?<?=$search?>&orderby=userid&orderlx=" class="<?=orac('userid')?>">会员ID</a></th>
            <th align="center"><a href="?<?=$search?>&orderby=username&orderlx=" class="<?=orac('username')?>">会员名</a></th>
