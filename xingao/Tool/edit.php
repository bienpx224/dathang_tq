...
//通用验证:允许修改的表和字段：必须验证，否则会员能随便修改所有数据
$chk=0; $ppt=$LG['Tool.ppt_2'];

if(!$chk&&$table=='baoguo' && ($fieldEdit=='content') )
{
	permissions('baoguo_ed',0,'manage','');
	$chk=1;
}
...
if($smt||!$typ)
{
	...
	$xingao->query("update {$table} set {$save} where {$fieldId}='{$id}' {$Xwh}");SQLError('更新');
...
{
	case '1'://----------------------------------修改备注---------------------------------------------
		$content=FeData($table,$fieldEdit,"{$fieldId}='{$id}' {$Xwh}");
		?>
