...
$pervar='member_ed,member_se,member_re';require_once($_SERVER['DOCUMENT_ROOT'].'/xingao/incluce/session.php');
$headtitle=cadd($_GET['key'])." 会员充值记录";
...
if($lx=='del')
{
	...
	$xingao->query("delete from money_czbak where {$where} ");//删除用户日志
...
if($so)
{
	...
	$search.="&so={$so}&key={$key}&type={$type}&stime={$stime}&etime={$etime}&fromtable={$fromtable}&fromid={$fromid}&showTyp={$showTyp}&fromCurrency={$fromCurrency}&toCurrency={$toCurrency}";
}
...
$query="select * from money_czbak where {$where} {$order}";
            ...
            <div class="form-group">
              <div class="col-md-0">
                <select  class="form-control input-small select2me" name="type" data-placeholder="类型">
                  <option></option>
                  <option value="member"  <?=$type=='member'?' selected':''?>>会员</option>
                  <option value="baoguo"  <?=$type=='baoguo'?' selected':''?>>包裹</option>
                  <option value="manage"  <?=$type=='manage'?' selected':''?>>后台</option>
            ...
            <div class="form-group">
                <div class="col-md-0">
                  <div class="input-group input-xmedium date-picker input-daterange" data-date-format="yyyy-mm-dd">
                    <input type="text" class="form-control input-small" name="stime" value="<?=$stime?>" placeholder="充值时间-起">
                    <span class="input-group-addon">-</span>
                    <input type="text" class="form-control input-small" name="etime" value="<?=$etime?>" placeholder="充值时间-止">
                  </div>
                 </div>
              </div>
        ...
        <table class="table table-striped table-bordered table-hover" >
          <thead>
            <tr>
              ...
              <th align="center"><a href="?<?=$search?>&orderby=type&orderlx=" class="<?=orac('type')?>">充值/退款类型</a></th>
              ...
              <th align="center"><a href="?<?=$search?>&orderby=addtime&orderlx=" class="<?=orac('addtime')?>">充值时间</a></th>
...
?>
           <tr class="odd gradeX">
              ...
              <td align="center"><?=money_cz($rs['type'])?></td>
...
?> 
     <tr class="odd gradeX">
      ...
      <td align="center">
        <strong>
        <?php 
        $query="select count(*) as total,sum(`toMoney`) as toMoney,toCurrency from money_czbak where {$where} {$where_m}  group by toCurrency";
        ...
        <strong>
          <?php 
          $query="select count(*) as total,sum(`fromMoney`) as fromMoney,fromCurrency from money_czbak where {$where} {$where_m} group by fromCurrency";
          $sql=$xingao->query($query);
          while($rs=$sql->fetch_array())
          {
...
<?php }?>
            ...
            <tr class="odd gradeX">
              ...
              <strong>
<?php 
$query="select count(*) as total,sum(`toMoney`) as toMoney,toCurrency from money_czbak where {$where} group by toCurrency";
$sql=$xingao->query($query);
while($rs=$sql->fetch_array())
{
	if(spr($rs['toMoney'])){echo spr($rs['toMoney']).' '.cadd($rs['toCurrency']).'<br>';}
...
?>
              ...
              <strong>
<?php 
$query="select count(*) as total,sum(`fromMoney`) as fromMoney,fromCurrency from money_czbak where {$where}  group by fromCurrency";
$sql=$xingao->query($query);
while($rs=$sql->fetch_array())
{
	if(spr($rs['fromMoney'])){echo spr($rs['fromMoney']).' '.cadd($rs['fromCurrency']).'<br>';}
