...
//template/article_list.php?classid=23
...
$query="select id,url{$LT},path,title{$LT},edittime,addtime,titlecolor from article where checked=1 and classid in ({$allclassid}) {$order}";
...
?>
                  ...
                  <span style="float:left"><a href="<?=$rs['url'.$LT]?cadd($rs['url'.$LT]):pathLT($rs['path'])?>" target="_blank"  style="color:<?=cadd($rs['titlecolor'])?>" title="<?=fnCharCount(cadd($rs['title'.$LT]))>80?cadd($rs['title'.$LT]):''?>"><?=leng($rs['title'.$LT],80,"...");?></a></span>
