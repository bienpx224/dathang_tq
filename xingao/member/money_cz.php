...
{ 
	...
	$token->is_token('money_cz',$tokenkey); //验证令牌密钥
	if (!$type){exit ("<script>alert('请选择付款类型！');goBack();</script>");}	
	if (!$username){exit ("<script>alert('请输入会员名或会员ID！');goBack();</script>");}	
	if ($new_money<=0){exit ("<script>alert('充值必须大于0！');goBack();</script>");}	
	if (!$currency){exit ("<script>alert('请选择币种！');goBack();</script>");}	

	//获取用户名和用户ID
	if($username){
		$query="select username,userid,money,currency from member  where userid='".$username."' or username='".$username."'  {$myMember}";
		...
		{ 
			 ...
			 //充值
			 MoneyCZ($rs['userid'],$fromtable,$fromid,$fromMoney=$new_money,$fromCurrency=$currency,
			 $title,$content,$type,$ddno=0,$operator=$Xuserid);
			 
			 $mr=FeData('member','money,currency',"userid='{$rs['userid']}'");
			 
			 $ts= "充值成功，已充<font class=red>".$new_money."</font>".$XAmc."，目前账户有".spr($mr['money']).$mr['currency'];
			 $ts.= ' <a href="money_czbak.php?so=1&key='.$rsusername.'" target="_blank">充值记录</a>';
		}
	 ...
	 $token->drop_token("money_cz"); //处理完后删除密钥
}

//生成令牌密钥(为安全要放在所有验证的最后面)
$token=new Form_token_Core();
$tokenkey= $token->grante_token("money_cz");
  ...
  <form action="?" method="post" class="form-horizontal form-bordered" name="xingao">
    ...
    <div class="portlet">
      ...
      <div class="portlet-body form" style="display: block;"> 
        ...
        <div class="form-group">
          <label class="control-label col-md-3">充值</label>
          <div class="col-md-9  has-error">
            <input name="new_money" class="form-control  input-small"  maxlength="8" type="text" onkeyup="value=value.replace(/[^\d.]/g,'')" required value="<?=par($_GET['money'])?>" style="float:left;"/>
...
<?=openCurrency('',2,1)?>
        ...
        <div class="form-group">
         ...
         <div class="form-group">
          ...
          <div class="col-md-9">
            ...
            <span class="help-block">如果是退款,要认真填写,此填写将影响月结金额和数据统计结果</span>
          </div>
        </div>
       ...
       <div class="form-group">
          ...
          <div class="col-md-9">
            <input type="text" class="form-control" name="title" value="<?=cadd($_GET['title'])?>">
          </div>
        </div>
