...
if(!$rs['id'])
{
	exit ("<script>alert('信息已经被删除！');goBack('uc');</script>");
}
?>
      ...
      <div class="portlet-body">
        <ul class="chats">
          ...
          <li class="in"> <img class="avatar img-responsive" src="<?=$userimg?>" width="50" height="50"/>
            <div class="message"> <span class="arrow"></span> <a href="../member/form.php?lx=edit&userid=<?=$rs['userid']?>" class="name" target="_blank">
              <?=$rs['username']?>
              ...
              <?php if($rs['file']){?>
              <br>
              <button type="button"  class="btn btn-success" onClick="window.open('<?=$rs['file']?>');"><i class="icon-file"></i> 查看附件</button>
              <?php }?>
              </span> </div>
          </li>
          ...
          <li class="out"><!--后台--> 
            ...
            <div class="message"> <span class="arrow"></span> <a class="name">回复者ID:
              <?=$rs2['reply_userid']?>
              </a> <span class="datetime">
              ...
              <?php if($rs2['file']){?>
              <br>
              <button type="button"  class="btn btn-success" onClick="window.open('<?=$rs2['file']?>');"><i class="icon-file"></i> 查看附件</button>
              <?php }?>
              </span> </div>
          </li>
          <?php
        ...
        <form action="save.php" method="post" class="form-horizontal form-bordered" name="xingao">
          ...
          <input name="reply_userid" type="hidden" value="<?=$Xuserid?>">
          <input name="reply_username" type="hidden" value="<?=$Xusername?>">
          <input name="tokenkey" type="hidden" value="<?=$tokenkey?>">
          <div class="chat-form">
            <div class="input-cont" style="margin-right:0px;">
              <textarea  class="form-control" rows="4" name="content" placeholder="回复内容..." ></textarea>
            </div>
