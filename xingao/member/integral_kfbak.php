...
$pervar='member_ed,member_in,member_se';
require_once($_SERVER['DOCUMENT_ROOT'].'/xingao/incluce/session.php');
$headtitle=cadd($_GET['key'])." 会员扣分记录";
...
if($lx=='del')
{
	...
	$xingao->query("delete from integral_kfbak where {$where} ");//删除用户日志
...
//echo $search;exit();
$query="select * from integral_kfbak where {$where} {$order}";
            ...
            <div class="form-group">
                <div class="col-md-0">
                  <div class="input-group input-xmedium date-picker input-daterange" data-date-format="yyyy-mm-dd">
                    <input type="text" class="form-control input-small" name="stime" value="<?=$stime?>" placeholder="扣分时间-起">
                    <span class="input-group-addon">-</span>
                    <input type="text" class="form-control input-small" name="etime" value="<?=$etime?>" placeholder="扣分时间-止">
                  </div>
                 </div>
              </div>
        ...
        <table class="table table-striped table-bordered table-hover" >
          <thead>
            <tr>
              ...
              <th align="center"><a href="?<?=$search?>&orderby=type&orderlx=" class="<?=orac('type')?>">扣分类型</a></th>
               <th align="center"><a href="?<?=$search?>&orderby=fromtable&orderlx=" class="<?=orac('fromtable')?>">用途</a> </th>
               <th align="center"><a href="?<?=$search?>&orderby=title&orderlx=" class="<?=orac('title')?>">扣分说明</a> </th>
              
              <th align="center"><a href="?<?=$search?>&orderby=integral&orderlx=" class="<?=orac('integral')?>">扣分</a></th>
              <th align="center"><a href="?<?=$search?>&orderby=money&orderlx=" class="<?=orac('money')?>">抵消金额</a></th>
              ...
              <th align="center"><a href="?<?=$search?>&orderby=addtime&orderlx=" class="<?=orac('addtime')?>">扣分时间</a></th>
...
?>
            ...
            <tr class="odd gradeX">
              ...
              <th align="center"><strong>
				<?php 
					$fe=FeData('integral_kfbak',"count(*) as total,sum(`integral`) as integral,sum(`money`) as money","{$where}");
