...
$pervar='member_in';require_once($_SERVER['DOCUMENT_ROOT'].'/xingao/incluce/session.php');
$headtitle="会员加积分";
$alonepage=1;//单页形式
...
{ 
	...
	if($username){
		...
		{ 
			 //加送分
			 integralCZ($rs['userid'],$fromtable,$fromid,$new_integral,$title,$content,$type,$operator=$Xuserid);
			 
			 $ts= "加积分成功，已加<font class=red>".$new_integral."</font>分，目前有".($rs['integral']+$new_integral)."分";
			 $ts.= ' <a href="integral_czbak.php?so=1&key='.$rsuserid.'" target="_blank">加积分记录</a>';
		}
	 ...
	 $token->drop_token("integral_cz"); //处理完后删除密钥
}

//生成令牌密钥(为安全要放在所有验证的最后面)
$token=new Form_token_Core();
$tokenkey= $token->grante_token("integral_cz");
  ...
  <form action="?" method="post" class="form-horizontal form-bordered" name="xingao">
    ...
    <div class="portlet">
      ...
      <div class="portlet-body form" style="display: block;"> 
        ...
        <div class="form-group">
          <label class="control-label col-md-3">加积分</label>
          <div class="col-md-9  has-error">
        ...
        <div class="form-group">
          <label class="control-label col-md-3">加积分类型</label>
          <div class="col-md-9  has-error">
         ...
         <div class="form-group">
          <label class="control-label col-md-3">退分原因</label>
          <div class="col-md-9">
            ...
            <span class="help-block">如果是退积分,请认真填写</span>
          </div>
        </div>
       <div class="form-group">
