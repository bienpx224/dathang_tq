...
if($so==1)
{
	...
	if($key){$where.=" and (ydh like '%{$key}%' or mobile like '%{$key}%' or email like '%{$key}%' or userid='".CheckNumber($key,-0.1)."' or lpid='".CheckNumber($key,-0.1)."' or username like '%{$key}%')";}
...
//echo $search;exit();
$query="select * from lipei where {$where} {$order}";
...
<!-- BEGIN PAGE TITLE & BREADCRUMB-->
      ...
      <!--<ul class="page-breadcrumb breadcrumb">
        <button type="button" class="btn btn-info" onClick="location.href='form.php';"><i class="icon-plus"></i> 申请理赔 </button>
        ...
        <div class="collapse navbar-collapse navbar-ex1-collapse">
          <form class="navbar-form navbar-left"  method="get" action="?">
            ...
            <div class="form-group">
              <input type="text" name="key" class="form-control input-msmall popovers" data-trigger="hover" data-placement="right"  data-content="理赔ID/运单号/电话/邮箱/会员ID/会员名 (可留空)" value="<?=$key?>">
            </div>
            <div class="form-group">
              <div class="col-md-0">
        ...
        <table class="table table-striped table-bordered table-hover" >
          <thead>
            <tr>
              ...
              <th align="center"><a href="?<?=$search?>&orderby=ydh&orderlx=" class="<?=orac('ydh')?>">运单号</a>/<a href="?<?=$search?>&orderby=types&orderlx=" class="<?=orac('types')?>">类型</a></th>
             
              <th align="center"><a href="?<?=$search?>&orderby=money&orderlx=" class="<?=orac('money')?>">赔付金</a></th>
               <th align="center"><a href="?<?=$search?>&orderby=mobile&orderlx=" class="<?=orac('mobile')?>">联系方式</a></th>
...
?>
            <tr class="odd gradeX <?=spr($rs['status'])==2?'gray2':''?>">
			 ...
			 <?=$rs['lpid']?><br>
              ...
              <td align="center" valign="middle"><?php if(spr($rs['status'])==2){?><?=spr($rs['money'])?> <?=$XAmc?><?php }else{echo '未完成';}?></td>
...
<?=showUsername($rs['username'],$rs['userid'])?>                           
         ...
         if($zhi){
		 ?>
          <div class="gray modal_border"> <strong>说明：</strong>
            <?php 
			echo leng($zhi,100,"...");
			Modals($zhi,$title='说明',$time=$rs['addtime'],$nameid='content'.$rs['lpid'],$count=100);
			?>
           </div>
        <?php }?>
        <!---->
         ...
         if($zhi){
		 ?>
          <div class="gray modal_border"> <strong>要求：</strong>
            <?php 
			echo leng($zhi,100,"...");
			Modals($zhi,$title='要求',$time=$rs['addtime'],$nameid='requ'.$rs['lpid'],$count=100);
			?>
           </div>
        <?php }?>
        <!---->
        ...
        <!--凭证-->
        <?php if($rs['img']){?>
        <div class="gray modal_border"><!-- <strong>凭证：</strong>-->
        ...
        <?php }?>
		 ...
		 ?>
          <div class="gray modal_border"> <strong>回复：</strong>
            <?php 
			echo leng($zhi,100,"...");
			Modals($zhi,$title='回复',$time=$rs['replytime'],$nameid='reply'.$rs['lpid'],$count=100);
			?>
           </div>
        <?php }?>
        <!---->
