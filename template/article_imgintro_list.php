...
{
?>
              <li> <a href="<?=$rs['url'.$LT]?cadd($rs['url'.$LT]):pathLT($rs['path'])?>"  target="_blank" class="img-cont"> <img src="<?=ImgAdd($rs['img'.$LT])?>" onload="AutoResizeImage(120,120,this)"> </a>
    <div class="text-cont"> <strong> <a href="<?=$rs['url'.$LT]?cadd($rs['url'.$LT]):pathLT($rs['path'])?>" target="_blank"  style="color:<?=cadd($rs['titlecolor'])?>" title="<?=fnCharCount(cadd($rs['title'.$LT]))>25?cadd($rs['title'.$LT]):''?>"><?=leng($rs['title'.$LT],25,"...");?></a> </strong> <span class="time"><?=DateYmd($rs['edittime'],2,$rs['addtime'])?> </span>
      <p class="remark" style="text-indent:0px;"> <?=leng($rs['intro'.$LT],70,"...")?> </p>
    </div>
  </li>
